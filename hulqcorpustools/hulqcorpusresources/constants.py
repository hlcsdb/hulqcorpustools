...
class FileFormat(Enum):
    """possible text formats for a doc to be in
        ...
        keyboard characters 
        'APA_Unicode': APA (Americanist Phonetic Alphabet) written with Unicode
        characters
    ...
    @staticmethod
    def file_formats():
        file_formats = {FileFormat.STRAIGHT, FileFormat.APA_UNICODE, FileFormat.ORTHOGRAPHY}
        return file_formats
    ...
    def from_string(cls, *args):
        ...
        elif 'apa_unicode'.casefold() in args or 'fileformat.apa-unicode'.casefold() in args:
            cls = FileFormat.APA_UNICODE
        elif 'orthography'.casefold() in args or 'fileformat.orthography'.casefold() in args:
            cls = FileFormat.ORTHOGRAPHY
        elif 'english'.casefold() in args or 'fileformat.english'.casefold() in args:
            cls = FileFormat.ENGLISH
        else:
            raise ValueError('"' + str(*args) + '"' + 
            ' is not a valid file format-- perhaps check spelling?')
        return cls

    def to_string(self):
        if self == FileFormat.APA_UNICODE:
            file_format_str = 'APA-unicode'
        else:
...
class TransliterandFile():
    ...
    **kwargs):
        ...
        self.source_folder = source_path.parent
        self.source_format = source_format
        self.suffix = source_path.suffix
        self.target_format = target_format

        if kwargs.get('target_folder'):
            self.target_folder = Path(kwargs.get('target_folder'))

        else:
            self.target_folder = self.source_folder

        # if a target destination folder is provided
        self.target_filename = Path(self.source_path.stem + 
                                            ' ' +
        ...
        self.target_path = Path(self.target_folder / self.target_filename)

        # if a target filename is provided
        if kwargs.get('target_path'):
            self.target_path = Path(kwargs.get('target_path'))
            self.target_folder = self.target_path.parent
            self.target_filename = Path(self.target_path.name)
